...
{
    public partial class hezuo1 : System.Web.UI.Page
    {
        ...
        private void GridBind()
        {
            ...
            {
                where.Append(" and ol3 like '%'+?+'%'");
                arrayList.Add(CheckSql.Filter(this.txtcol3.Text.Trim()));
                sql += " and ol3 like '%" + this.txtcol3.Text.Trim() + "%' ";
            }
        ...
        {
            Response.Redirect("hezuo1Add.aspx?id=" + "" + "", false);
        }
        ...
        protected void btn编辑_Click(object sender, EventArgs e)
        {
            ...
            for (int k = 0; k < this.GridView1.Rows.Count; k++)
            {
                ...
                if (ifTick)
                {
                    ...
                    Response.Redirect("hezuo1Add.aspx?id=" + tickedFormNo + "&temp1=" + temp1 + "&temp2=" + temp2 + "&temp3=" + temp3 + "&temp4=" + temp4 + "&temp5=" + temp5 + "&temp6=" + temp6 + "&temp7=" + temp7 + "", false);
                    break;
                }
            }
        ...
        {
            hezuo1List hezuo1List = new hezuo1List();
            var formNoList = new List<string>();
            for (int k = 0; k < this.GridView1.Rows.Count; k++)
            {
                var checkedControl = (CheckBox)GridView1.Rows[k].FindControl("chk");
                ...
                if (ifTick)
                {
                    ...
                    var lblcol4 = ((Label)GridView1.Rows[k].FindControl("lblcol2")).Text;
                    ...
                    var tickedFormNo = ((Label)GridView1.Rows[k].FindControl("lblID")).Text; // 若該列有被選取，取出被選的單號
                    formNoList.Add(tickedFormNo);
                    hezuo1List.DeleteData(int.Parse(tickedFormNo));
                }
            }
        ...
        protected void btnExport_Click(object sender, EventArgs e)
        {
            ...
            string str = "合作单位管理" + dt.ToString("yyyyMMddhhmmss");
            str = str + ".xlsx";
            //GridBind("download");
            ...
            MojoCube.Api.NPOIHelp.NPOIHelp.ExportXlsx("/Admin/DownLoadTemplates/合作单位管理下载模板.xlsx", downloadTable, str);
        }
        ...
        protected void GridView1_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            ...
            {
                //if (((Label)e.Row.FindControl("lbl合作单位证照资料")).Text.Trim() == "")
                //{
                //    ((CheckBox)e.Row.FindControl("Chk合作单位证照资料")).Visible = false;
                //    ((HyperLink)e.Row.FindControl("HyperLinkcol合作单位证照资料")).Visible = false;
                //    ((Label)e.Row.FindControl("Label合作单位证照资料")).Visible = true;
                //}
                //if (((Label)e.Row.FindControl("lbl法定代表人资料")).Text.Trim() == "")
                //{
                //    ((CheckBox)e.Row.FindControl("Chk法定代表人资料")).Visible = false;
                //    ((HyperLink)e.Row.FindControl("HyperLinkcol法定代表人资料")).Visible = false;
                //    ((Label)e.Row.FindControl("Label法定代表人资料")).Visible = true;
                //}
                //if (((Label)e.Row.FindControl("lbl开户许可证资料")).Text.Trim() == "")
                //{
                //    ((CheckBox)e.Row.FindControl("Chk开户许可证资料")).Visible = false;
                //    ((HyperLink)e.Row.FindControl("HyperLinkcol开户许可证资料")).Visible = false;
                //    ((Label)e.Row.FindControl("Label开户许可证资料")).Visible = true;
                //}
                if (((Label)e.Row.FindControl("lbl合作单位证照资料")).Text.Trim() == "1")
                {
                    ((LinkButton)e.Row.FindControl("lbtn1")).Visible = true;
                    ((Label)e.Row.FindControl("Label合作单位证照资料")).Visible = false;
                }
                else
                {
                ...
                if (((Label)e.Row.FindControl("lbl法定代表人资料")).Text.Trim() == "1")
                {
                    ((LinkButton)e.Row.FindControl("lbtn2")).Visible = true;
                    ((Label)e.Row.FindControl("Label法定代表人资料")).Visible = false;
                }
                else
                {
                ...
                if (((Label)e.Row.FindControl("lbl开户许可证资料")).Text.Trim() == "1")
                {
                    ((LinkButton)e.Row.FindControl("lbtn3")).Visible = true;
                    ((Label)e.Row.FindControl("Label开户许可证资料")).Visible = false;
                }
                else
                {
                ...
                ((HyperLink)e.Row.FindControl("HyperLink1")).NavigateUrl = "hezuo1Add.aspx?id=" + strid + "&temp1=" + temp1 + "&temp2=" + temp2 + "&temp3=" + temp3 + "&temp4=" + temp4 + "&temp5=" + temp5 + "&temp6=" + temp6 + "&temp7=" + temp7 + "&model=view";
                ...
                if (((Label)e.Row.FindControl("lblcol12")).Width.Value < 140)
                {
                    ((Label)e.Row.FindControl("lblcol12")).Width = 140;
                }
                if (((Label)e.Row.FindControl("lblcol21")).Width.Value < 110)
                {
                    ((Label)e.Row.FindControl("lblcol21")).Width = 110;
                }
        ...
        protected void GridView1_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            ...
            {
                string tempfilename = "合作单位证照资料" + System.DateTime.Now.ToString("yyyyMMddhhmmss") + ".zip";
                btnDownLoad_Click("hezuo11", rowIDs, tempfilename);
            }
            if (e.CommandName == "lbtn12")
            {
                string tempfilename = "合同扫描件" + System.DateTime.Now.ToString("yyyyMMddhhmmss") + ".zip";
                btnDownLoad_Click("hezuo12", rowIDs, tempfilename);
            }
            if (e.CommandName == "lbtn13")
            {
                string tempfilename = "合同其他附件" + System.DateTime.Now.ToString("yyyyMMddhhmmss") + ".zip";
                btnDownLoad_Click("hezuo13", rowIDs, tempfilename);
            }
        }
