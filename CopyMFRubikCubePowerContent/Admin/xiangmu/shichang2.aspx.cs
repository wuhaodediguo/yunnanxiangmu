...
{
    public partial class shichang2 : System.Web.UI.Page
    {
        ...
        private void GridBind()
        {
            ...
            adminPager.TableName = "t_shichangview2";
            adminPager.strGetFields = "*";
            ArrayList arrayList = new ArrayList();
            StringBuilder where = new StringBuilder();
            where.Append(" where 1=1 and col39 = '1' ");
            string sql = "select * from t_shichangview2 where 1= 1 and col39 = '1' ";
            ...
            if (this.txtcol5.Text.Trim() != "")
            {
                ...
                sql += " and col24 like '%" + this.txtcol5.Text.Trim() + "%' ";
            }
            ...
            this.GridView1.DataSource = adminPager.GetTable34(memoryPage);
        ...
        {
            Response.Redirect("shichang2Add.aspx?id=" + "" + "", false);
        }
        ...
        protected void btn编辑_Click(object sender, EventArgs e)
        {
            ...
            for (int k = 0; k < this.GridView1.Rows.Count; k++)
            {
                ...
                if (ifTick)
                {
                    ...
                    Response.Redirect("shichang2Add.aspx?id=" + tickedFormNo + "&temp1=" + temp1 + "&temp2=" + temp2 + "&temp3=" + temp3 + "&temp4=" + temp4 + "&temp5=" + temp5 + "&temp6=" + temp6 + "&temp7=" + temp7 + "", false);
                    break;
                }
            }
        ...
        protected void btnExport_Click(object sender, EventArgs e)
        {
            ...
            string str = "中标管理" + dt.ToString("yyyyMMddhhmmss");
            str = str + ".xlsx";
            //GridBind("download");
            ...
            for (int i = 0; i < dtAll.Rows.Count; i++)
            {
                ...
                dr["A8"] = dtAll.Rows[i]["col24"].ToString();
                dr["A9"] = dtAll.Rows[i]["col25"].ToString();
                dr["A10"] = dtAll.Rows[i]["col26"].ToString();
                dr["A11"] = dtAll.Rows[i]["col27"].ToString();
                dr["A12"] = dtAll.Rows[i]["col28"].ToString();
                dr["A13"] = dtAll.Rows[i]["col29"].ToString();
                dr["A14"] = dtAll.Rows[i]["col30"].ToString();
                dr["A15"] = dtAll.Rows[i]["col31"].ToString();
                dr["A16"] = dtAll.Rows[i]["col32"].ToString();

                dr["A42"] = dtAll.Rows[i]["col43"].ToString();
                dr["A17"] = dtAll.Rows[i]["col33"].ToString();
                dr["A18"] = dtAll.Rows[i]["col34"].ToString();
                dr["A19"] = dtAll.Rows[i]["col35"].ToString();
                dr["A20"] = dtAll.Rows[i]["col36"].ToString();
                dr["A21"] = dtAll.Rows[i]["col40"].ToString();
                dr["A22"] = dtAll.Rows[i]["col41"].ToString();

                dr["A23"] = dtAll.Rows[i]["col37"].ToString();
                dr["A24"] = dtAll.Rows[i]["col38"].ToString();
                dr["A25"] = dtAll.Rows[i]["col23"].ToString();
               
                downloadTable.Rows.Add(dr);
            }

            MojoCube.Api.NPOIHelp.NPOIHelp.ExportXlsx("/Admin/DownLoadTemplates/中标管理下载模板.xlsx", downloadTable, str);
        }

        protected void btnUpload_Click(object sender, EventArgs e)
        {
            Response.Redirect("shangchuan.aspx?id=" + "shichang2" + "", false);
        ...
        protected List<shichangList> checkUploadData(DataTable dt)
        {
            ...
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                ...
                list.col25 = dt.Rows[i][8].ToString().Trim() == "" ? "" : Convert.ToDateTime(dt.Rows[i][8].ToString().Trim()).ToString("yyyy/MM/dd");
                list.col26 = dt.Rows[i][9].ToString().Trim() == "" ? "" : Convert.ToDateTime(dt.Rows[i][9].ToString().Trim()).ToString("yyyy/MM/dd");
                list.col27 = dt.Rows[i][10].ToString();
                list.col28 = dt.Rows[i][11].ToString();
                list.col29 = dt.Rows[i][12].ToString().Trim() == "" ? "" : Convert.ToDateTime(dt.Rows[i][12].ToString().Trim()).ToString("yyyy/MM/dd");
                list.col30 = dt.Rows[i][13].ToString().Trim() == "" ? "" : Convert.ToDateTime(dt.Rows[i][13].ToString().Trim()).ToString("yyyy/MM/dd");
                list.col31 = dt.Rows[i][14].ToString();
                list.col32 = dt.Rows[i][15].ToString();
                list.col33 = dt.Rows[i][16].ToString().Trim() == "" ? "" : Convert.ToDateTime(dt.Rows[i][16].ToString().Trim()).ToString("yyyy/MM/dd");
                list.col34 = dt.Rows[i][17].ToString();
                list.col35 = dt.Rows[i][18].ToString();
                list.col36 = dt.Rows[i][19].ToString();
                list.col37 = dt.Rows[i][20].ToString().Trim() == "" ? "" : Convert.ToDateTime(dt.Rows[i][20].ToString().Trim()).ToString("yyyy/MM/dd");
                list.col38 = dt.Rows[i][21].ToString();
                list.col23 = dt.Rows[i][22].ToString();
        ...
        protected void GridView1_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            ...
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                ((HyperLink)e.Row.FindControl("HyperLink1")).NavigateUrl = "shichang2Add.aspx?id=" + strid + "&temp1=" + temp1 + "&temp2=" + temp2 + "&temp3=" + temp3 + "&temp4=" + temp4 + "&temp5=" + temp5 + "&temp6=" + temp6 + "&temp7=" + temp7 + "&model=view";

                if (((Label)e.Row.FindControl("lbl中标通知书")).Text.Trim() == "1")
                {
                    ((LinkButton)e.Row.FindControl("lbtn1")).Visible = true;
                    ((Label)e.Row.FindControl("Label中标通知书")).Visible = false;
                }
                else
                {
                    ((LinkButton)e.Row.FindControl("lbtn1")).Visible = false;
                    ((Label)e.Row.FindControl("Label中标通知书")).Visible = true;
                }
                if (((Label)e.Row.FindControl("lbl履约保证金保函")).Text.Trim() == "1")
                {
                    ((LinkButton)e.Row.FindControl("lbtn2")).Visible = true;
                    ((Label)e.Row.FindControl("Label履约保证金保函")).Visible = false;
                }
                else
                {
                    ((LinkButton)e.Row.FindControl("lbtn2")).Visible = false;
                    ((Label)e.Row.FindControl("Label履约保证金保函")).Visible = true;
                }
                //if (((Label)e.Row.FindControl("lbl中标通知书")).Text.Trim() == "")
                //{
                //    ((CheckBox)e.Row.FindControl("Chk中标通知书")).Visible = false;
                //    ((HyperLink)e.Row.FindControl("HyperLinkcol20")).Visible = false;
                //    ((Label)e.Row.FindControl("Label中标通知书")).Visible = true;
                //}
                //if (((Label)e.Row.FindControl("lbl履约保证金保函")).Text.Trim() == "")
                //{
                //    ((CheckBox)e.Row.FindControl("Chk履约保证金保函")).Visible = false;
                ...
                string strlblcol42 = ((Label)e.Row.FindControl("lblcol43")).Text.Replace("年", "").Replace("月", "").Replace("日", "").Trim();
                if (strlblcol42 == "")
                {
                }
                else
                {
                    if (DateTime.Compare(Convert.ToDateTime(strlblcol42), Convert.ToDateTime(DateTime.Now.ToString("yyyy-MM-dd"))) <= 0)
                    {
                        if (strlblcol18 == "" && strlblcol19 == "")
                        {
                            ...
                            ((Label)e.Row.FindControl("lblcol43")).ForeColor = System.Drawing.Color.Red;
                            ((HyperLink)e.Row.FindControl("HyperLink1")).ForeColor = System.Drawing.Color.Red;
                            ((Label)e.Row.FindControl("lblcol40")).ForeColor = System.Drawing.Color.Red;
                            ((Label)e.Row.FindControl("lblcol41")).ForeColor = System.Drawing.Color.Red;
                        }
                ...
                //((Label)e.Row.FindControl("lblcol11")).Attributes.Add("style", "height:20px;text-align:right;");
                //((Label)e.Row.FindControl("lblcol14")).Attributes.Add("style", "height:20px;text-align:right;");
                //((Label)e.Row.FindControl("lblcol18")).Attributes.Add("style", "height:20px;text-align:right;");
                ...
                if (((Label)e.Row.FindControl("lblcol14")).Width.Value < 150)
                {
                    ((Label)e.Row.FindControl("lblcol14")).Width = 150;
                }
                if (((Label)e.Row.FindControl("lblcol15")).Width.Value < 120)
                {
                    ((Label)e.Row.FindControl("lblcol15")).Width = 120;
                }
                if (((Label)e.Row.FindControl("lblcol17")).Width.Value < 120)
                {
                    ((Label)e.Row.FindControl("lblcol17")).Width = 120;
                }
        ...
        protected void GridView1_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            ...
            {
                string tempfilename = "中标通知书" + System.DateTime.Now.ToString("yyyyMMddhhmmss") + ".zip";
                btnDownLoad_Click("shichang21", rowIDs, tempfilename);
            }
            if (e.CommandName == "lbtn12")
            {
                string tempfilename = "履约保证金保函" + System.DateTime.Now.ToString("yyyyMMddhhmmss") + ".zip";
                btnDownLoad_Click("shichang22", rowIDs, tempfilename);
            }
